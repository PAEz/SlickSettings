...
//
(function() {
    // If I want to be able to extend Bundle with new Types then I needed to make settings and Bundle global....cant help but think there'd be a better way for this
    // Wish I knew more about MooTools ;)
    //var settings1, Bundle1;
    ...
    Bundle = new Class({
        ...
        "initialize": function(params) {
            ...
            this.createDOM(); //WHHHEEEEEE!!!!!! JUGGLING ;()
            if(this.params.id !== undefined) {
                this.element.set("id", this.params.id);
            }

            if(this.params.name !== undefined) {
                var value = settings.get(this.params.name);

                if(value === undefined && this.params['default'] !== undefined) {
                    value = this.params['default'];
                    this.set(value);
                    settings.set(this.params.name, value);
                } else {
                    this.set(value, true);
                }
            }
            // The one thing I still cant figure, why does my spectrum addon need the following?\/
            if(this.params.value == undefined) this.params.value = value; // GAWD FREAKN DAM IT!!!!!!!!!!!!!!!!!!!!  Could we please have some consistency!!!
            this.setupDOM(); //WHHHEEEEEE!!!!!! JUGGLING ;()
            this.addEvents(); //WHHHEEEEEE!!!!!! JUGGLING ;()
    ...
    Bundle.PopupButton = new Class({
        ...
        "createDOM": function() {
            ...
            // convert arrays
            //console.debug(this.params.options)
    ...
    Bundle.Textarea = new Class({
        ...
        "setupDOM": function() {
            ...
            // Changed the param name from text to holder because that makes more sense
            if(this.params.holder !== undefined) {
                this.element.set("placeholder", this.params.holder);
                //this.params.searchString += this.params.holder + "•"; //Is including the holder text in the search really a good idea?
            }

            // I dont know...but this whole bit doesnt seem to work and is only going to get replaced with whats in store anywayz?
            /*            if (this.params.value !== undefined) {
                console.debug(this.element)
                this.element.appendText(this.params.value);
    ...
    this.Setting = new Class({
        "initialize": function(container) {
            ...
            // Moved types to intit to make it easier to extend with new Types
            this.types = {
